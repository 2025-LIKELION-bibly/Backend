...
public class ReadingSessionService {
    ...
    private List<BookSimpleResponse> bookInfo;
    ...
    public List<ReadingSessionResponse> getOngoingSessionsForUser(String userId) {
        ...
        List<ReadingSession> sessionEntities = readingSessionRepository.findByUser_UserId(userId);

        List<ReadingSessionResponse> sessionDtos = sessionEntities.stream()
                .map(session -> new ReadingSessionResponse(session, bookInfo, sessionId)) // DTO 생성자 호출
