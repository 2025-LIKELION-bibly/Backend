...
public class GroupController {
	...
	@Operation(
		...
		description = """
			...
			**회차 계산 로직:**
			- 현재 시간이 특정 배정의 기간(startDate ~ endDate) 내에 있으면 해당 회차 반환
			- 진행 중인 회차가 없으면 (모든 회차 종료) 마지막 회차 반환
	...
	@Operation(
		...
		description = """
			...
			**프로세스:**
			1. 시간 기준으로 현재 진행 중인 회차를 확인합니다
			2. 활성 모임원 수를 확인합니다
			3. 현재 라운드의 모든 회차를 완료했는지 확인합니다 (마지막 회차의 endDate < 현재 시간)
			4. 재시작 가능 여부를 반환합니다

			**재시작 가능 조건:**
			- 현재 라운드의 모든 회차를 완료한 경우
			- 마지막 회차의 독서 기간이 종료된 경우
			- 예: 4명의 모임원, 1라운드 4회차 완료 → 재시작 가능 (5~8회차 추가)
	...
	@Operation(
		summary = "모임 재시작",
		description = """
			현재 라운드의 모든 회차를 완료한 후 새로운 라운드를 시작합니다.

			**프로세스:**
			1. 요청자가 모임장인지 확인합니다
			2. 재시작 가능 여부를 확인합니다 (현재 라운드 모든 회차 완료)
			3. 기존 배정과 한줄평, 독서 기록을 모두 보존합니다
			4. 탈퇴한 모임원(WITHDRAWN)을 상태 그대로 유지합니다
			5. 활성 모임원의 책 선택을 확인하고, 선택하지 않은 경우 랜덤 배정합니다
			6. 기존 회차 이후에 새로운 회차를 추가로 생성합니다

			**검증 규칙:**
			- 모임장만 재시작할 수 있습니다 (M006)
			- 현재 라운드의 모든 회차를 완료해야 재시작 가능합니다

			**재시작 효과:**
			- 기존 배정, 한줄평, 독서 기록 모두 보존 
			- 탈퇴한 모임원 데이터 보존 (WITHDRAWN 상태 유지) 
			- 책을 선택하지 않은 모임원: 랜덤 책 자동 배정 (중복 제외)
			- 새로운 회차 추가 생성 (예: 1~4회차 보존 → 5~8회차 추가)
			- 모임 상태를 IN_PROGRESS로 변경

			**데이터 보존:**
			- 이전 회차의 모든 배정 정보, 한줄평, 독서 기록이 보존됩니다
			- 여러 라운드를 계속 진행할 수 있습니다
			"""
	)
	@ApiResponses(value = {
		@io.swagger.v3.oas.annotations.responses.ApiResponse(
